...
const API_KEY = import.meta.env.VITE_API_KEY || 'sk_prod_9f8e7d6c5b4a3f2e1d0c9b8a7f6e5d4c';
...
function App() {
    ...
    const loggedReasons = useRef(new Set());

    const simPos = useRef({ lat: 21.1458, lon: 79.0882 });


    const memoizedPath = useMemo(() => path, [path]);
    const memoizedRiskData = useMemo(() => riskData, [riskData]);
        ...
        newSocket.on('ride_update', (data) => {
            ...
            if (data.reasons && data.reasons.length > 0) {
                data.reasons.forEach(r => {
                    const baseReason = r.replace(/\(.*\)/, '').trim();
                    if (!loggedReasons.current.has(baseReason)) {
                        loggedReasons.current.add(baseReason);
                        addLog(`Risk Increase: ${r}`, 'RISK_UP');
                    }
                });
            }
        });
    ...
    const startSimulation = (type) => {
        ...
        const id = setInterval(() => {
            simTick.current += 1;
            const t = simTick.current;
            
            let speed = 15;
            let latDelta = 0.000158; // (21.1600 - 21.1458) / 90
            let lonDelta = 0.000020; // (79.0900 - 79.0882) / 90
            let accuracy = 8;
            let deviation = 0;

            if (type === 'BAD') {
                if (t <= 5) {
                    speed = 15; latDelta = 0.000158; lonDelta = 0.000020; accuracy = 8;
                } else if (t <= 10) {
                    speed = 22; latDelta = 0.000158; lonDelta = 0.000060; accuracy = 10;
                } else if (t <= 20) {
                    speed = 32; latDelta = 0.000050; lonDelta = 0.000250; accuracy = 12;
                } else {
                    speed = 40;
                    if ((t >= 22 && t <= 24) || (t >= 28 && t <= 29)) speed = 0;
                    else if (t === 21) speed = 38;
                    else if (t === 25) speed = 41;
                    else if (t === 26) speed = 43;
                    else if (t === 30) speed = 44;
                    latDelta = -0.000100; lonDelta = -0.000200; accuracy = 15;
                }
            }
            
            simPos.current.lat += latDelta;
            simPos.current.lon += lonDelta;
            ...
            <div className="flex-1 flex flex-col p-4 gap-4">
                ...
                <div className="grid grid-cols-3 gap-4 h-32">
                    ...
                    <div className="bg-slate-900 p-4 rounded-lg border border-slate-700 flex items-center justify-between">
                        ...
                        <AlertTriangle className={`w-12 h-12 ${rideState?.severity === 'ELEVATED' ? 'text-yellow-500' :
                            rideState?.severity === 'MONITORING' ? 'text-orange-500' :
                            rideState?.severity === 'ESCALATED' ? 'text-red-500' : 'text-slate-700'
                            }`} />
                    </div>
                ...
                <div className="flex-1 grid grid-cols-3 gap-4 min-h-0">
                    <div className="col-span-2 flex flex-col gap-4">
                        <div className="flex-1 min-h-0 bg-slate-900 rounded-lg border border-slate-700 overflow-hidden">
                            <MapView path={memoizedPath} currentPos={rideState?.location} startPos={rideState?.startLocation} endPos={rideState?.endLocation} />
                        </div>
                        <div className="h-48 shrink-0">
                            <RiskChart data={memoizedRiskData} />
                        </div>
                    </div>
